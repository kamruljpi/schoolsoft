@inject('studentAdmitCardForm','Erp\Forms\StudentAdmitCardForm')
            ...
            <div class="col-md-12">
                <div class="student-box-header">
                    <div class="col-md-6 col-xs-5">
                        <span class="glyphicon glyphicon-user " aria-hidden="true"></span>{{ trans('sidebar.get_admit_card') }}
                    </div>
                    <div class="col-md-6 col-xs-7 snt">
                        <ul class="breadcrumb text-right">
                            ...
                            <li class="active">{{ trans('sidebar.get_admit_card') }}</li>
                        </ul>
                    </div>
                </div>
            </div>
                            ...
                            <div class="box">
                                ...
                                <div class="box-body">
                                    ...
                                    <div class="custom_row">
                                         <h2 class="form_title">
                                            {{ trans('sidebar.get_admit_card') }}
                                        </h2>
                                    </div>
                                   

                                    {!! formFields($studentAdmitCardForm) !!}
                                        <div class="row custom_padding">
                                        {!! Form::submit('Student Admit Card',['class'=>'btn btn-primary','id'=>'student-admit-card-btn','readonly'=>'readonly']) !!}

                                        </div>
                                   </div>
                                </div><!-- /.box-body -->
                                 <div id="student-admit-card-report">
...
@section('scripts')
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            $('#student-admit-card-btn').click(function () {

                var examinationId = $('#examination_id').find('option:selected').val();
                var studentClassId = $('#student_class_id').find('option:selected').val();
                var sectionId = $('#section_id').find('option:selected').val();
                var studentId = $('#student_id').find('option:selected').val();
                var host = window.location.origin ;

                if(examination_id == 0|| studentClassId == 0 || sectionId == 0 || studentId == 0){
                    alert('please select exam name, class, section and student');
                }else {
                    $.ajax({
                        url: host + '/report/student-admit-card-report/' +examinationId+'/'+studentClassId+'/'+ sectionId+'/'+studentId,
                        type: "GET", // not POST, laravel won't allow it
                        success: function(data){
                            /*alert(data);*/
                            $data = $(data); // the HTML content your controller has produced

                            $('#student-admit-card-report').html($data);
                        }
                    });
                }
